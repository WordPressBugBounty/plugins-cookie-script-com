...
if(!(!!window.CookieScript && !!window.CookieScript.instance)) {
  window.CookieScript = function () {
    ...
    this.initTime = null;
    this.saveTime = '2025-08-20 08:10:43';
    this.consentTime = 1718109524;
    ...
    var mapJSKeyToDBReasonConsentModeValidation = {"enabledConsentMode":"como_not_enabled","correctGoogleTemplateTrigger":"gtm_incorrect_trigger","gtagDefaultConsent":"gtag_no_default_consent","gtagCorrectOrder":"gtag_incorrect_order","gtagRequiredCategory":"gtag_no_required_category"};
    var showEvent = 'load';
    ...
    var _isAuth = function () {
            var scriptAllowUrl = 'cookie-script.com/s/435af40da4ad165d8f87ae4866b927e2.js';
    ...
    var _verifyGoogleConsentMode = function () {
      ...
      if(googleConsentEnabled) {
        window.CookieScriptData.enabledConsentMode = true;
        if(!window.CookieScriptData.useGoogleTemplate) {
          var isVerifyGoogleConsentMode = true;
          ...
          if(allDefaultIndex.length > 1) {
            defaultGoogleConsentModeFromUserSet = true;
          }
          const allRequiredCategory = requiredConsentCategory.every(category => usedConsentCategory.includes(category));
          window.CookieScriptData.gtagDefaultConsent = defaultGoogleConsentModeFromUserSet && defaultConsentIndex !== null;
          if(window.CookieScriptData.gtagDefaultConsent) {
            window.CookieScriptData.gtagRequiredCategory = allRequiredCategory;
            if(configIndex !== null) {
              window.CookieScriptData.gtagCorrectOrder = defaultConsentIndex < configIndex;
            } else if(gtmJsEventIndex !== null) {
              window.CookieScriptData.gtagCorrectOrder = defaultConsentIndex < gtmJsEventIndex;
            } else {
              window.CookieScriptData.gtagCorrectOrder = defaultConsentIndex === 0;
            }

            if(window.CookieScriptData.gtagRequiredCategory === false) {
              isVerifyGoogleConsentMode = false;
              reason = mapJSKeyToDBReasonConsentModeValidation.gtagRequiredCategory
            } else {
              if(window.CookieScriptData.gtagCorrectOrder === false) {
                isVerifyGoogleConsentMode = false;
                reason = mapJSKeyToDBReasonConsentModeValidation.gtagCorrectOrder
              }
            }
          } else {
            isVerifyGoogleConsentMode = false;
            reason = mapJSKeyToDBReasonConsentModeValidation.gtagDefaultConsent;
          }
          window.CookieScriptData.isVerifyGoogleConsentMode = isVerifyGoogleConsentMode;
        } else {
          if(window.CookieScriptData.isVerifyGoogleConsentMode === false) {
            reason = mapJSKeyToDBReasonConsentModeValidation.correctGoogleTemplateTrigger;
          }
        }
      } else {
        window.CookieScriptData.enabledConsentMode = false;
        window.CookieScriptData.isVerifyGoogleConsentMode = false;
      }
